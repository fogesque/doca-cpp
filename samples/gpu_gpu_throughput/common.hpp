...
inline const std::string configsFilename = "gpu_gpu_throughput_configs.yaml";
...
inline std::tuple<SampleConfigPtr, error> ParseSampleConfigs(const std::string & configFilename)
{
    ...
    if (auto clientNode = yamlConfig["client"]) {
        ...
        if (clientNode["gpu-pcie-bdf"]) {
            cfg->clientCfg.gpuPcieBdfAddress = clientNode["gpu-pcie-bdf"].as<std::string>();
        }
...
inline void PrintSampleConfigs(const SampleConfigPtr cfg)
{
    ...
    std::println("    GPU PCIe BDF:       {}", cfg->clientCfg.gpuPcieBdfAddress);
