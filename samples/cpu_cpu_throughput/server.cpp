...
///
/// @brief CPU server service that counts received buffers for throughput measurement.
///
class ThroughputCounter : public doca::rdma::IRdmaStreamService
{
public:
...
int main()
{
    ...
    // Build CPU streaming server
    std::println("[Server] Creating CPU RDMA streaming server");

    auto [server, serverErr] = doca::rdma::RdmaStreamServer::Create()
                                   .SetDevice(device)
